		...
		"enum" => {
			if let Expression::Identifier(id) = value {
				...
				call.arguments.first().and_then(|a| {
					if let Argument::Identifier(id) = a { Some(id.name.to_string()) } else { None }
				}).unwrap_or_else(|| "unknown_enum".to_string())
			} else { "unknown_enum".to_string() }
		}
		"message" => {
			if let Expression::Identifier(id) = value { return id.name.to_string(); }
			if let Expression::ArrowFunctionExpression(arrow) = value {
				for stmt in &arrow.body.statements {
					if let Statement::ExpressionStatement(es) = stmt {
						if let Expression::Identifier(bid) = &es.expression { return bid.name.to_string(); }
...
fn find_fields_call<'a>(body: &'a [Statement<'a>], name: &str) -> Option<&'a CallExpression<'a>> {
	...
	for member in &class.body.body {
		let ClassElement::StaticBlock(sb) = member else { continue };
		for stmt in &sb.body {
			...
			let AssignmentTarget::StaticMemberExpression(m) = &assign.left else { continue };
...
fn fields_to_json(call: &CallExpression, mapping: &HashMap<String, String>) -> Value {
	...
	for el in &arr.elements {
		let ArrayExpressionElement::ObjectExpression(obj) = el else { continue };
		let no = match get_obj_prop(obj, "no") {
			Some(p) => match &p.value { Expression::NumericLiteral(n) => n.value as u64, _ => continue },
			None => continue,
		};
		...
		let t = get_obj_prop(obj, "T").map(|p| {
			...
			let resolved = mapping.get(&raw).cloned().unwrap_or(raw);
			match kind.as_str() {
				"scalar" => match &p.value {
					Expression::NumericLiteral(n) => json!(n.value as u64),
					_ => json!(resolved),
				},
				_ => json!(resolved),
			}
		...
		if let Some(t) = t { entry["T"] = t; }
		if repeated { entry["repeated"] = json!(true); }
		if opt { entry["opt"] = json!(true); }
		if let Some(oneof) = oneof { entry["oneof"] = oneof; }
		if let Some(map) = map_val { entry["map"] = map; }
...
fn extract_enums(body: &[Statement]) -> Value {
	...
	for stmt in body {
		...
		if m.property.name.as_str() != "setEnumType" { continue; }
		...
		let name_arg = match args.next() { Some(a) => a, None => continue };
		...
		let enum_name = match name_arg {
			...
			Argument::TemplateLiteral(t) if t.expressions.is_empty() && t.quasis.len() == 1 => {
				t.quasis[0].value.cooked.as_deref()
		...
		for el in &values_arr.elements {
			...
			let no = obj.properties.iter().find_map(|p| {
				...
				{ Some(n.value as u64) } else { None }
			});
			let name = obj.properties.iter().find_map(|p| {
				if let ObjectPropertyKind::ObjectProperty(op) = p
					&& util::get_prop_key_name(&op.key) == Some("name")
				{ util::get_string_value(&op.value).map(|s| s.to_string()) } else { None }
			});
			if let (Some(no), Some(name)) = (no, name) {
				values.push(json!({"no": no, "name": name}));
			}
		}

		map.insert(enum_name, json!(values));
	}
...
fn find_objs_in_expr<'a>(expr: &'a Expression<'a>, out: &mut Vec<&'a ObjectExpression<'a>>) {
	match expr {
		...
		Expression::SequenceExpression(seq) => { for e in &seq.expressions { find_objs_in_expr(e, out); } }
...
fn find_packet_maps(body: &[Statement]) -> PacketMaps {
	...
	for stmt in body {
		match stmt {
			...
			Statement::VariableDeclaration(vd) => {
				for decl in &vd.declarations {
					if let Some(init) = &decl.init { find_objs_in_expr(init, &mut objs); }
				}
			}
			_ => {}
		}
	...
	for obj in &objs {
		...
		if has_c && !has_s { c_pairs = pairs; }
		else if has_s && !has_c { s_pairs = pairs; }
		else if has_c && has_s { appended_pairs = pairs; }
	}

	PacketMaps { c_pairs, s_pairs, appended_pairs }
}
...
fn collect_deps(
	...
	for el in &arr.elements {
		...
		let tp = match get_obj_prop(obj, "T") { Some(p) => &p.value, None => continue };
		...
		let dep = match kind {
			...
			"map" => match tp {
				Expression::ObjectExpression(o) => {
					...
					if scalar_names.contains(&vt) || mapping.contains_key(&vt) || visited.contains(&vt) {
						continue;
					}
					vt
				}
		...
		if !scalar_names.contains(&dep) && !mapping.contains_key(&dep) && !visited.contains(&dep) && !deps.contains(&dep) {
			deps.push(dep);
		}
	}
	deps
...
pub fn extract_bundle(source: &str) -> Result<String, String> {
	...
	let all_pairs: Vec<&[(String, String)]> = vec![&maps.c_pairs, &maps.s_pairs, &maps.appended_pairs];
	...
	for pairs in &all_pairs {
		for (_key, prop) in *pairs {
			...
			if let Some(fc) = find_fields_call(body, &runtime) {
				let fields = fields_to_json(fc, &name_mapping);
				messages.insert(tn.clone(), json!({
					"typeName": tn,
					"fields": fields,
				}));
			}
